...
# 1. 입력 파일에서 데이터 로드
file_path = os.path.join(os.path.dirname(__file__), "results", "430_stat.txt")
...
if not rotation_df.empty:
    sns.scatterplot(data=rotation_df, x="time", y="similarity", hue="pair", style="pair", s=100, ax=ax5)
    ax5.set_title("Rotation Similarity over Time")
...
if not translation_df.empty:
    sns.scatterplot(data=translation_df, x="time", y="similarity", hue="pair", style="pair", s=100, ax=ax6)
    ax6.set_title("Translation Similarity over Time")
